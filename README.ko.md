...
# 현재 폴더 대신 특정 프로젝트 폴더 지정 (<폴더>/.claude/agents에 기록)
...
같은 엔진 함수(`show` / `set_agent` / `set_main` / `set_fcc_tier`)를 그대로 재사용하는 가벼운 Flask 대시보드 — 로직·백업 동작이 CLI와 동일합니다. 드롭다운, 저장 범위 선택, 심링크 가드 프롬프트, 그리고 **한/영 전환 버튼**을 갖춘 브라우저 UI. 두 개의 탭:

- **모델** — 메인 모델·에이전트별 모델·(fcc면) 티어 매핑을 지정. 상주 서버는 '현재 폴더'가 없으므로 **프로젝트 스코프는 폴더 경로 입력이 필수**입니다(프로젝트 선택 시 입력란 표시). 빈 경로는 거부되어 서버 루트에 잘못 기록되지 않습니다.
- **세션** — `/resume` 스타일 목록. `~/.claude/projects/*/*.jsonl`에서 각 세션의 `ai-title`·마지막 프롬프트·실제 `cwd`를 스캔해 최근 활동순으로 보여줍니다(검색 + 세션 ID 클릭 복사, 재개는 터미널 `claude --resume <id>`). 각 행의 **⚙ 모델 설정** 버튼 → 모델 탭으로 이동. 단, Claude Code의 "프로젝트"는 실행 `cwd`라, 대부분의 세션이 홈 디렉터리(=전역 agents 폴더)에서 실행되면 subagent 설정은 사실상 **전역(모든 세션 공통)** 입니다 — 탭 상단에 이를 명시합니다.

```bash
python3 ~/.claude/skills/subagent/dashboard.py                    # localhost 전용 :8097 (안전 기본값)
SUBAGENT_DASH_HOST=0.0.0.0 python3 .../dashboard.py               # LAN 노출 (신뢰된 네트워크 전용)
```

기본은 `127.0.0.1` 바인딩입니다. `SUBAGENT_DASH_HOST`(+ 선택 `SUBAGENT_DASH_PORT`)로 바꿉니다.

systemd로 상주화 — 예시 유닛 포함:
...
> **시크릿은 나가지 않음.** 대시보드는 모델 *이름*만 오갑니다 — `show()`에 API 키가 없고, 페이지도 그 외엔 아무것도 전송하지 않습니다. 쓰기 엔드포인트는 **설계상 인증이 없어서**(로컬 사용자 소유 파일을 수정) 기본적으로 localhost에 바인딩합니다 — 신뢰하는 네트워크에서만 `SUBAGENT_DASH_HOST=0.0.0.0`을 설정하세요.
...
├── dashboard.py                          # 선택: 엔진을 재사용하는 Flask 웹 UI(한/영 전환)
└── subagent-dashboard.service.example    # 상주 호스팅용 systemd 유닛
```
