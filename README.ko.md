...
```bash
python3 ~/.claude/skills/subagent/dashboard.py     # 0.0.0.0:8097 서빙
```
...
> **시크릿은 나가지 않음.** 대시보드는 모델 *이름*만 오갑니다 — `show()`에 API 키가 없고, 페이지도 그 외엔 아무것도 전송하지 않습니다. 쓰기 엔드포인트는 설계상 인증이 없으므로(로컬 사용자 소유 파일을 수정) **신뢰된 LAN/localhost에만** 바인딩하세요.
