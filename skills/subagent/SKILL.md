...
## 웹 대시보드 (선택)
- `python3 ~/.claude/skills/subagent/dashboard.py` → Flask 앱, `http://<LAN-IP>:8097` (신뢰된 LAN/localhost 전용).
- systemd 상주: `subagent-dashboard.service` (재부팅 생존, `MemoryMax=128M`).
- 기능: 메인 모델·에이전트별 모델·(fcc면) 티어 매핑을 브라우저에서 드롭다운으로 지정. 저장 범위(전역/프로젝트) 선택, 심링크 가드 모달, 우상단 **한/영 전환** 버튼.
- `subagent_tool.py`의 `show`/`set_agent`/`set_main`/`set_fcc_tier`를 그대로 재사용 → CLI와 동일 로직·동일 백업.
- ⚠ **API 키·자격증명은 노출/전송하지 않는다** — 모델명만 다룸(`show()`에 키 미포함).
